...
Exists filter also works with relations. If a property of the filter is a `toMany` relation, then `exists[property]=true` will return all items for which the relation has at least 1 item, whereas `exists[property]=false` will return all items whose `property` relation has 0 items.
